...
void FUnLuaEditorToolbar::BindCommands()
{
	...
	CommandList->MapAction(Commands.CreateLuaTemplate,
	                       FExecuteAction::CreateRaw(this, &FUnLuaEditorToolbar::CreateLuaTemplate_Executed));
	CommandList->MapAction(Commands.CopyAsRelativePath,
	                       FExecuteAction::CreateRaw(this, &FUnLuaEditorToolbar::CopyAsRelativePath_Executed));
	CommandList->MapAction(Commands.CreateDefaultLuaTemplate,
	                       FExecuteAction::CreateRaw(this, &FUnLuaEditorToolbar::CreateLuaDefualtTemplate_Executed));
	CommandList->MapAction(Commands.CopyAsDefaultModuleName,
	                       FExecuteAction::CreateRaw(this, &FUnLuaEditorToolbar::CopyAsDefualtModuleName_Executed));
	CommandList->MapAction(Commands.BindToLua,
	                       FExecuteAction::CreateRaw(this, &FUnLuaEditorToolbar::BindToLua_Executed));
	CommandList->MapAction(Commands.UnbindFromLua,
	                       FExecuteAction::CreateRaw(this, &FUnLuaEditorToolbar::UnbindFromLua_Executed));
	CommandList->MapAction(Commands.RevealInExplorer,
	                       FExecuteAction::CreateRaw(this, &FUnLuaEditorToolbar::RevealInExplorer_Executed));
	CommandList->MapAction(Commands.RevealInIDE,
	                       FExecuteAction::CreateRaw(this, &FUnLuaEditorToolbar::RevealInIDE_Executed));
}
...
void FUnLuaEditorToolbar::BuildToolbar(FToolBarBuilder& ToolbarBuilder, UObject* InContextObject)
{
	...
	ToolbarBuilder.AddComboButton(
		...
		FOnGetContent::CreateLambda([&, Blueprint, InContextObject]()
		{
			...
			{
				MenuBuilder.AddMenuEntry(Commands.CopyAsRelativePath, NAME_None,
				                         LOCTEXT("CopyAsRelativePath", "Copy as Relative Path"));
				MenuBuilder.AddMenuEntry(Commands.CreateLuaTemplate, NAME_None,
				                         LOCTEXT("CreateLuaTemplate", "Create Lua Template"));
				MenuBuilder.AddMenuEntry(Commands.CopyAsDefaultModuleName, NAME_None,
				                         LOCTEXT("CopyAsDefaultModuleName", "Copy as Default Module Name"));
				MenuBuilder.AddMenuEntry(Commands.CreateDefaultLuaTemplate, NAME_None,
				                         LOCTEXT("CreateDefaultLuaTemplate", "Create Default Lua Template"));
				MenuBuilder.AddMenuEntry(Commands.RevealInExplorer, NAME_None,
	...
	BPMenu->AddDynamicSection("UnLua", FNewToolMenuDelegate::CreateLambda([this](UToolMenu* ToolMenu)
	{
		...
		if (GraphNodeCtx && GraphNodeCtx->Graph)
		{
			if (GraphNodeCtx->Graph->GetName() == "GetModuleName")
			{
				...
				UnLuaSection.AddEntry(FToolMenuEntry::InitMenuEntryWithCommandList(
					FUnLuaEditorCommands::Get().RevealInExplorer, CommandList,
					LOCTEXT("RevealInExplorer", "Reveal in Explorer")));
			}
		}
...
void FUnLuaEditorToolbar::BindToLua_Executed() const
{
	...
	const auto Ok = FBlueprintEditorUtils::ImplementNewInterface(
		Blueprint, FTopLevelAssetPath(UUnLuaInterface::StaticClass()));
#endif
	if (!Ok)
		return;
...
void FUnLuaEditorToolbar::CreateLuaTemplate_Executed()
{
	...
	if (FPaths::FileExists(FileName))
	{
		UE_LOG(LogUnLua, Warning, TEXT("%s"),
		       *FText::Format(LOCTEXT("FileAlreadyExists", "Lua file ({0}) is already existed!"), FText::FromString(
	...
	for (auto TemplateClass = Class; TemplateClass; TemplateClass = TemplateClass->GetSuperClass())
	{
		...
		Content = Content.Replace(TEXT("TemplateName"), *TemplateName)
		                 .Replace(TEXT("XActor"), *TemplateName)
...
void FUnLuaEditorToolbar::RevealInExplorer_Executed()
{
	...
	else
	{
		...
		NotificationInfo.Text = FText::Format(
			LOCTEXT("FileNotExist", "The file {0} does not exist."), FText::FromString(FileName));
...
void FUnLuaEditorToolbar::CreateLuaDefualtTemplate_Executed()
{
	...
	for (auto TemplateClass = Class; TemplateClass; TemplateClass = TemplateClass->GetSuperClass())
	{
		...
		if (UnLuaSuperClass)
		{
			...
			const FString SuperFileName = FString::Printf(TEXT("%s%s"), *SuperPath, *SuperClassName).Replace(
				TEXT("/"), TEXT("."));

			Content = Content.Replace(TEXT(" = UnLua.Class()"), *FString::Printf(
				                          TEXT(", Super = UnLua.Class(\"%s\")"), *SuperFileName))
